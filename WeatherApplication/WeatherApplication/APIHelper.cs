...
namespace WeatherApplication
{
    class APIHelper
    {
        ...
        public static async Task<WeatherInfo> GetCurrentWeatherData(string searchCity)
        {
            try
            {
                string key = "22203868ce0f4c0fd047ba21349a54e1";
                string city_name = AppHelper.sharedPref.GetString(Application.Context.GetString(Resource.String.city_pref_key), Application.Context.GetString(Resource.String.default_city));
                ...
                string unit_name = AppHelper.sharedPref.GetString(Application.Context.GetString(Resource.String.unit_pref_key), Application.Context.GetString(Resource.String.default_unit));


                UnitsOfMeasurement units = (UnitsOfMeasurement)Enum.Parse(typeof(UnitsOfMeasurement), unit_name);

                string lang_name = AppHelper.sharedPref.GetString(Application.Context.GetString(Resource.String.lang_pref_key), Application.Context.GetString(Resource.String.default_lang));
            ...
            {
#if DEBUG
                Toast.MakeText(Application.Context, "Data Parcing Error: " + ex.Message, ToastLength.Long).Show();
                Console.WriteLine("=================");
        ...
        public static async Task<List<WeatherInfo>> GetForecastWeatherData(string searchCity)
        {
            try
            {
                ...
                for (int i = 0; i < cnt; i++)
                {
                    ...
                    {
                        // get picture
                        info.Icon = await HttpHelper.GetWeatherIcon(data["list"][0]["weather"][0]["icon"].ToString());
                    }).Wait();
                    infoLst.Add(info);
                }
